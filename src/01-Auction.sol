...
contract KeyManager {
    ...
    function onchain_Bootstrap(bytes memory _xPub, bytes memory att)
    public
    {
	require(xPub.length == 0); // only once
	Suave.verifySgx(address(this), 
			keccak256(abi.encodePacked("xPub", _xPub)), att);
	xPub = _xPub;
    }
}
...
contract SealedAuction is LeakyAuction, KeyManager {
	...
	for (uint i = 0; i < bidders.length; i++) {
	    ...
	    uint bid = abi.decode(PKE.decrypt(xPriv, ciphertext), (uint));
	    if (bid > best) {
    ...
    function onchain_Finalize(uint secondPrice_, bytes memory att) public {
	require(block.number > auctionEndTime);
	Suave.verifySgx(address(this),
			keccak256(abi.encodePacked("2ndprice",secondPrice_)),
			att);
