...
abstract contract KeyManagerBase {
    ...
    function derivedPriv() public virtual returns (bytes32);

    function derivedPub(address a) public virtual view returns (bytes memory);
...
contract KeyManager_v0 is KeyManagerBase {
    ...
    mapping(address => bytes) public derivedPub_;

   function derivedPub(address a) public override view returns (bytes memory) {
        return derivedPub_[a];
    }
    ...
    function derivedPriv() public override returns (bytes32) {
...
contract KeyManager_v1_Upgradeable is KeyManagerBase {
    BIP32 public bip32;
    uint256 version;
    address owner;
    
    mapping(uint256 => bytes32) public approvedMRenclaves;
    mapping(uint256 => address) public xPubHistorical;

    constructor(address _Suave, bytes32 approvedMRenclave_) {
        Suave = IAndromeda(_Suave);
        bip32 = new BIP32(Suave);
        version = 1;
        approvedMRenclaves[version] = approvedMRenclave_;
        Suave.setTrustedMrEnclave(approvedMRenclave_);
        owner = msg.sender;
    }
    ...
    function approvedNewMRenclave(bytes32 approvedMRenclave_) onlyOwner public { //TODO change onlyOwner -> to majority voting
        version = version + 1;
        approvedMRenclaves[version] = approvedMRenclave_;
        Suave.setTrustedMrEnclave(approvedMRenclave_);
        xPub = address(0);
    ...
    // Because we are using hardened derivation, for each
    // contract we will need someone to sign it off chain
    mapping(uint256 => mapping(address => bytes)) public derivedPubs;

    function derivedPub(address a) public override view returns (bytes memory) {
        return derivedPubs[version][a];
    }
    ...
    // 1. Bootstrap phase
    function offchain_Bootstrap() public returns (address _xPub, bytes memory att) {
        require(xPubHistorical[version] == address(0));
        bytes32 seed = Suave.localRandom();
        bytes32 xPrivKey = bip32.newFromSeed(abi.encodePacked(seed)).key;
        _xPub = Secp256k1.deriveAddress(uint256(xPrivKey));
        setSeed(seed);
    ...
    function onchain_Bootstrap(address _xPub, bytes memory att) public {
        require(xPubHistorical[version] == address(0), "already bootstrapped");
        require(Suave.verifySgx(address(this), keccak256(abi.encodePacked("xPub", _xPub)), att), "remote attestation verification failed");
        xPub = _xPub;
        xPubHistorical[version] = _xPub;
    }

    // 2. New node register phase
    // Mapping to nonzero indicates valid Kettle
    mapping(uint256 => mapping(address => bytes)) registry;

    function offchain_Register() public returns (address addr, bytes memory myPub, bytes memory att) {
        require(keccak256(registry[version][addr]) == keccak256(bytes("")), "already registered");
    ...
    function onchain_Register(address addr, bytes memory myPub, bytes memory att) public {
        require(keccak256(registry[version][addr]) == keccak256(bytes("")), "already registered");
        require(Suave.verifySgx(address(this), keccak256(abi.encodePacked("myPub", myPub, addr)), att), "remote attestation verification failed");
        registry[version][addr] = myPub;
    }
    ...
    function offchain_Onboard(address newkettle) public returns (bytes memory ciphertext) {
        bytes32 r = Suave.localRandom();
        return PKE.encrypt(registry[version][newkettle], r, abi.encodePacked(getSeed()));
    }

    event Onboard(uint256 version, address addr, bytes ciphertext);

    function onchain_Onboard(address addr, bytes memory ciphertext) public {
        // Note: nothing guarantees all ciphertexts on chain are valid
        emit Onboard(version, addr, ciphertext);
    }
    ...
    function finish_Onboard(bytes memory ciphertext) public {
        ...
        bytes32 seed = abi.decode(PKE.decrypt(myPriv, ciphertext), (bytes32));
        bytes32 _xPriv = bip32.newFromSeed(abi.encodePacked(seed)).key;
        require(Secp256k1.deriveAddress(uint256(_xPriv)) == xPub, "private key wrong");
        setSeed(seed);
    }
}
