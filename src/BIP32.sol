...
contract BIP32 {
    ...
    function newFromSeed(bytes memory seed) public view returns (ExtendedPrivateKey memory) {
        ...
        require(
            seed.length == 16 || seed.length == 32 || seed.length == 64, "BIP32: seed length must be 16, 32 or 64 bytes"
        );
        bytes memory output =
            hasher.sha512(abi.encodePacked(Utils.bytesToHexString(abi.encodePacked(BIP32_DERIVATION_DOMAIN, seed))));
    ...
    function deriveChildKeyPairFromSeed(bytes memory seed, uint32 index)
        external
        view
        returns (ExtendedPrivateKey memory xPriv, ExtendedPublicKey memory xPub)
    {
    ...
    function deriveChildKeyPair(ExtendedPrivateKey memory parent, uint32 index)
        public
        view
        returns (ExtendedPrivateKey memory xPriv, ExtendedPublicKey memory xPub)
    {
        ...
        if (index >= 0x80000000) {
            ...
            bytes memory output = hasher.sha512(
                abi.encodePacked(Utils.bytesToHexString(abi.encodePacked(parent.attributes.chainCode, data)))
            );
            (bytes32 secret_key, bytes32 chain_code) = split(output);
    ...
    function deriveChildKeyPairFromPath(bytes memory seed, string memory path)
        external
        view
        returns (ExtendedPrivateKey memory xPriv, ExtendedPublicKey memory xPub)
    {
        ...
        require(
            pathBytes.length > 0 || pathBytes[0] == bytes1("m") || pathBytes[0] == bytes1("M"), "BIP32: invalid path"
        );
        ...
        for (uint256 i = 2; i < pathBytes.length; i++) {
    ...
    function derivePubKeyFromParentPubKey(ExtendedPublicKey memory parent, uint32 index)
        external
        view
        returns (ExtendedPublicKey memory xPub)
    {
        ...
        bytes memory output =
            hasher.sha512(abi.encodePacked(Utils.bytesToHexString(abi.encodePacked(parent.attributes.chainCode, data))));
        (bytes32 secret_key, bytes32 chain_code) = split(output);
        ExtendedKeyAttributes memory extKeyAttr = ExtendedKeyAttributes(index, chain_code);
        xPub = ExtendedPublicKey(PKE.derivePubKey(secret_key), extKeyAttr);
    }
