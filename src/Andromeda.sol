...
contract Andromeda is IAndromeda, DcapDemo {
	...
	bytes32 keyhash = keccak256(abi.encodePacked([codehash(), key]));
        bytes memory cdata = abi.encodePacked([keyhash, value]);
	...
	bytes32 keyhash = keccak256(abi.encodePacked([codehash(), key]));
        (bool success, bytes memory value) = VOLATILEGET_ADDR.staticcall(abi.encodePacked([keyhash]));
        require(success);
    ...
    function attestSgx(bytes32 appData) external override returns (bytes memory) {
        (bool success, bytes memory attestBytes) = ATTEST_ADDR.staticcall(abi.encodePacked(codehash(), msg.sender, appData));
        require(success);
        return attestBytes;
    }

    function verifySgx(address caller, bytes32 appData, bytes memory att) public view returns (bool) {
        bytes memory userdata = abi.encode(address(this), abi.encodePacked(codehash(), caller, appData));
