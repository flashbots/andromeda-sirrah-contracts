...
interface SM {
    function getService(string memory service_name, bytes memory config)
        external
        returns (bytes32 handle, bytes memory err);
    ...
    function callServiceWithContext(Context memory env, bytes32 handle, bytes memory cdata)
        external
...
contract ExternalServiceImpl {
    ...
    fallback(bytes calldata cdata) external returns (bytes memory) {
        if (_volatile_handle == bytes32(0x0)) {
            (bool s_ok, bytes memory s_data) =
                SM_ADDR.staticcall(abi.encodeWithSelector(SM.getService.selector, service, config));
        ...
        (bool c_ok, bytes memory c_data) =
            SM_ADDR.staticcall(abi.encodeWithSelector(SM.callService.selector, _volatile_handle, cdata));
