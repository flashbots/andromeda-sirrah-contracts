...
contract CryptoTest is Test {
    function test_signing() public {
        ...
        address a = Secp256k1.deriveAddress(uint256(sk));
        bytes32 digest = keccak256(abi.encodePacked("hi"));
        bytes memory sig = Secp256k1.sign(uint256(sk), digest);
    ...
    function test_address() public {
        ...
        (uint256 qx, uint256 qy) = Secp256k1.derivePubKey(uint256(sk));
        address a = Secp256k1.deriveAddress(uint256(sk));
    ...
    function test_encryption() public {
        bytes32 secretKey = bytes32(uint256(0x4646464646464646464646464646464646464646464646464646464646464646));
        Curve.G1Point memory pub = Curve.g1mul(Curve.P1(), uint256(secretKey));

        bytes memory message = bytes("hello there suave,      #32bytes");

        // Encrypt the message to the auction contract
        bytes32 r = bytes32(uint256(0x1231251));
        bytes memory ciphertext = PKE.encrypt(abi.encodePacked(pub.X, pub.Y), r, message);
